...
function initBasicFunctions() {
   ...
   $('.logo-visual').each(function () {
      var logoVisual = $(this);
      var logoVisualClientName = logoVisual.find('[data-loop-text]').attr('data-loop-text');
      logoVisual.find('[data-loop-text]').css("--cursor-speed", " " + logoVisualClientName.length + "s");
   });
   ...
   $('[data-cyclecards-init]').each(function(){
      ...
      cycleCards.find('[data-cyclecard-hover]').on('mouseenter click', function() {
         ...
         if(cycleCards.find('[data-cyclecard-hover][data-cyclecard-id="' + dataCycleCardID + '"]').attr('data-cyclecard-status') != 'active') {
            ...
            cycleCards.find('[data-cyclecard-id="' + dataCycleCardID + '"]').attr('data-cyclecard-status', 'active').siblings().attr('data-cyclecard-status', 'not-active');
            cycleCards.find('[data-cyclecard-image][data-cyclecard-id="' + dataCycleCardID + '"]').css('z-index', cycleCardsIndex);

            gsap.fromTo(cycleCards.find('[data-cyclecard-image][data-cyclecard-id="' + dataCycleCardID + '"]'), {
               yPercent: 100,
               scale: 1,
               rotate: 0.001,
               transformOrigin: "right top"
            ...
            gsap.fromTo(cycleCards.find('[data-cyclecard-image][data-cyclecard-id="' + dataCycleCardIDPrev + '"]'), {
               yPercent: 0,
               scale: 1,
               rotate: 0.001,
               transformOrigin: "center top"
   ...
   $('[data-slideshow-status="active"]').each(function(){
      ...
      function nextSlideshowSlide() {
         ...
         function nextSlideshowSlideSwitch() {
            ...
            if(slideshow.find('[data-slideshow-slide-status="active"]').next().length > 0) {
               slideshow.find('[data-slideshow-slide-status="active"]').attr('data-slideshow-slide-status', 'transition-out').next().attr('data-slideshow-slide-status', 'active').css('z-index', slideShowSlideIndex);
               setTimeout(function() {
                  slideshow.find('[data-slideshow-slide-status="transition-out"]').attr('data-slideshow-slide-status', 'not-active');
               }, 800);
            }
            else {
               slideshow.find('[data-slideshow-slide-status="active"]').attr('data-slideshow-slide-status', 'transition-out');
               slideshow.find('[data-slideshow-slide-status]').first().attr('data-slideshow-slide-status', 'active').css('z-index', slideShowSlideIndex);
               setTimeout(function() {
                  slideshow.find('[data-slideshow-slide-status="transition-out"]').attr('data-slideshow-slide-status', 'not-active');
               }, 800);
            }
 ...
 function initCheckTheme() {
   ...
   function checkThemeSection() {
      ...
      for (var i = 0; i < themeSections.length; i++) {
         ...
         var themeObserverOffsetBottom = ($("header .btn-hamburger").position().top + ($("header .btn-hamburger").innerHeight() / 2));
         ...
         if (themeSectionTop <= themeObserverOffsetBottom && themeSectionBottom >= themeObserverOffsetBottom) {
            // Check [data-theme-section]
            var themeSectionActive = $(themeSection).attr('data-theme-section');
            if ($('[data-theme-nav-bottom]').attr('data-theme-nav-bottom') == themeSectionActive) {
            } else {
               $('[data-theme-nav-bottom]').attr('data-theme-nav-bottom', themeSectionActive);
            }

            // Check [data-bg-section]
            var bgSectionActive = $(themeSection).attr('data-bg-section');
            if ($('[data-bg-nav-bottom]').attr('data-bg-nav-bottom') == bgSectionActive) {
            } else {
...
function initVimeoPlayer() {
   ...
   $('[data-vimeo-player-target]').each(function(index){
      ...
      playerID.find('[data-anchor-target]').attr('data-anchor-target', videoScrollIndexIDHashtag);
      ...
      function checkIndexSection() {
         ...
         for (var i = 0; i < indexSections.length; i++) {
            ...
            if (indexSectionTop <= indexObserverOffset && indexSectionBottom >= indexObserverOffset) {
               ...
               $('[data-thumb-video-status][data-slide-item-index="' + (parseInt(singleSectionIndex)) +  '"]').attr('data-thumb-video-status', 'active').find('video').trigger('play');
               $('[data-thumb-video-status][data-slide-item-index="' + (parseInt(singleSectionIndex)) +  '"]').siblings().attr('data-thumb-video-status', 'not-active').find('video').trigger('pause');
            }
         }
      }
